...
'''
TODOs:
1.reset fields after adding new book to database-done
2. find a book - done
3. new Book Id cannot be empty
4. chnage return gUI to table like findBook-done
'''
...
def formatShowUserInfo(*args):
	...
	checkBookList = library.getUserInfo(user)
	for widget in Frame7.winfo_children():
		widget.destroy()
	if(len(checkBookList) == 0):
		messagebox.showinfo(message='No more books are checked out')
		return

	showCheckedBookData(checkBookList)
...
def showCheckedBookData(checkBookList):
	master = Frame7
	master.grid(row=4,column=0,columnspan=5)

	Label(master,width = 24, text="Select").grid(row=1,column=0)
	Label(master,width = 20, text="Book ID").grid(row=1,column=1)
	Label(master,width = 20, text="Title").grid(row=1,column=2)
	Label(master,width = 20, text = "Checkedout Date").grid(row=1,column=3)
	c = 0
	r = 5
	i = 0
	varReturnList = []
	for rec in checkBookList:
		print(checkBookList[i][0])
		
		var = IntVar()
		c = Checkbutton(master, width = 20, bg="white",variable=var).grid(row=r,column=c)
		varReturnList.append(var)
		
		Label(master, width = 20, bg="white", text = checkBookList[i][0]).grid(row=r,column = 1)
		Label(master, width = 20, bg="white", text = checkBookList[i][1]).grid(row=r,column = 2)
		Label(master, width = 20, bg="white", text = checkBookList[i][2]).grid(row=r,column = 3)
		
		r = r + 1
		i = i + 1
		
	bSubmit = ttk.Button(Frame7,text="Submit",command=lambda:formatReturnData(checkBookList,varReturnList))
	bSubmit.grid(row=r+1,column =3,sticky=E)
...
for r in range(20):
    ...
    Frame3.rowconfigure(r,weight=1) #return book main frame
    Frame4.rowconfigure(r,weight=1)
    Frame5.rowconfigure(r,weight=1) #find a book main frAME
    Frame6.rowconfigure(r,weight=1)
    Frame7.rowconfigure(r,weight=1)
...
for c in range(3):
	...
	Frame7.columnconfigure(c,weight=1)
for c in range(3,5):
	Frame3.columnconfigure(c,weight=1)
	Frame5.columnconfigure(c,weight=1)
	Frame6.columnconfigure(c,weight=1) #Find book result display
	Frame7.columnconfigure(c,weight=1) #return books result display
...
varUserSelected.trace("w",formatShowUserInfo)
